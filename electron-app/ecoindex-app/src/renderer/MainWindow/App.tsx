...
function TheApp() {
  ...
  const runJsonSaveAndCollect = async (saveAndCollect = false) => {
    ...
    showHidePopinDuringProcess(`${t('Courses Measure (Full mode)')} started 🚀`)
    try {
    ...
    const installPuppeteerBrowser = async () => {
      return new Promise<string | boolean>((result, reject) => {
        window.electronAPI
          ...
          .catch(value => {
            frontLog.error(`installPuppeteerBrowser Failed 🚫`, value)
            reject(value)
          })
      })
    }
    ...
    window.electronAPI.changeLanguageInFront((lng: string) => {
      try {
        i18nResources.changeLanguage(lng, (err, t) => {
          if (err) return frontLog.error('something went wrong loading', err)
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {false && (
            <>
              ...
              <div>isNodeInstalled: {isNodeInstalled ? 'true' : 'false'}</div>
              <div>
                isLighthouseEcoindexPluginInstalled:{' '}
                {isLighthouseEcoindexPluginInstalled ? 'true' : 'false'}
              </div>
              <div>isNodeVersionOK: {isNodeVersionOK ? 'true' : 'false'}</div>
            </>
          )}
          {!isNodeInstalled && (
            ...
            <AlertBox variant="bug" title="Report error">
              <div className="flex items-center justify-between gap-4">
                <span>
                  ...
                  <a href="mailto:renaud@greenit.fr" className="underline">
                    renaud@greenit.fr
                  </a>{' '}
                  🙏
                </span>
                <SimpleTooltip
                  tooltipContent={
                    <p>{t('Copy application informations to clipboard.')}</p>
                  }
                >
                  <Button
                    id="bt-report"
          ...
          {appReady && (
            <>
              <Card className="w-full border-primary">
                <CardHeader>
                  <CardTitle>{t('1. Select ouput folder')}</CardTitle>
                  <CardDescription>
                    {t('Specify where to execute the measures.')}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex w-full items-center gap-2">
                    <Input id="filePath" value={workDir} type="text" readOnly />
                    <Button
                      type="button"
                      id="btn-file"
                      disabled={!appReady}
              ...
              <TypographyP className={`w-full`}>
                {t('Choose the type of measure you want to do.')}
              </TypographyP>
