 ...
 * @returns
 */
function convert(type: string, value: number) {
  switch (type) {
    case 'water-to-pack':
...
function convertCourseResults(
  flows: Flows,
  course: Course,
): CourseResultConverted {
  const obj: CourseResultConverted = {
    ...
    summary: {
      'eco-index-grade': `undefined`,
      'eco-index-score': -1,
      'eco-index-water': -1,
      'eco-index-water-equivalent': `undefined`,
      'eco-index-ghg': -1,
      'eco-index-ghg-equivalent': `undefined`,
    },
    pages: [],
  }
  // get pages
...
function convertPagesResults(lhr: LH.Result): Summary {
